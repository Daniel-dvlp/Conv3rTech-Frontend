...
const NewProjectModal = ({ isOpen, onClose, onSave }) => {
  ...
  const validateField = (name, value) => {
    ...
    if (name === "fechaInicio" && value && value < todayStr)
      error = "La fecha de inicio no puede ser menor al día actual";
    ...
    if (
      ...
      error = "La fecha de fin no puede ser antes que la de inicio";
    return error;
  };
  ...
  const handleChange = (e) => {
    ...
    if (name === "fechaInicio" || name === "fechaFin") {
      ...
      if (fin && inicio && fin < inicio) {
        setErrors((prev) => ({
          ...prev,
          fechaFin: "La fecha de fin no puede ser antes que la de inicio",
        }));
      } else {
  ...
  const handleSubmit = (e) => {
    ...
    if (projectData.fechaInicio && projectData.fechaInicio < todayStr)
    ...
    if (
      ...
      projectData.fechaInicio &&
      projectData.fechaFin < projectData.fechaInicio
    )
      newErrors.fechaFin =
    ...
    for (const mat of projectData.materiales) {
      ...
      if (prod) {
        const totalAsignado =
          Number(mat.cantidad) +
          projectData.sedes.reduce((acc, sede) => {
  ...
  const handleAddMaterialToSede = (item) => {
    setSedeForm((prev) => {
      if (prev.materialesAsignados.some((m) => m.item === item.item))
        return prev;
      return {
        ...prev,
        materialesAsignados: [
  ...
  const handleAddServicioToSede = (servicio) => {
    setSedeForm((prev) => {
      if (prev.serviciosAsignados.some((s) => s.servicio === servicio.servicio))
        return prev;
      return {
        ...prev,
        serviciosAsignados: [
          ...prev.serviciosAsignados,
          { servicio: servicio.servicio, cantidad: 0, precio: servicio.precio },
        ],
      };
    });
  };
  ...
  const getStockDisponible = (nombreProducto) => {
    ...
    const cantidadMateriales = projectData.materiales
      .filter((m) => m.item === nombreProducto)
      .reduce((acc, m) => acc + Number(m.cantidad), 0);
    const cantidadSedes = projectData.sedes.reduce((acc, sede) => {
      const found = sede.materialesAsignados.find(
        (m) => m.item === nombreProducto
      );
      return acc + (found ? Number(found.cantidad) : 0);
    }, 0);
        ...
        <div className="space-y-4">
          ...
          {items.map((item, index) => {
            ...
            if (listName === "servicios") {
              const serv = serviciosDisponibles.find(
            ...
            else if (stockDisponible > 0 && stockDisponible < 10)
              stockColor = "text-yellow-500 font-semibold";
            return (
              <div
                key={index}
                className="grid grid-cols-[1fr,auto,auto,auto,auto] gap-3 items-center"
              >
                {listName === "materiales" ? (
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    handleListChange(
                      index,
                      { target: { name: "cantidad", value: val } },
                      listName
                    );
                  }}
                  ...
                  required
                  min="1"
                  max={listName === "materiales" ? stockDisponible : undefined}
                  disabled={listName === "materiales" && stockDisponible === 0}
                />
                <input
                  type="number"
                  name="precio"
        ...
        <div className="flex gap-2 mb-4">
          ...
          {projectData.cliente && (
            <button
              ...
              onClick={() => {
                const sedesPrecargadas = getSedesFromCliente(
                  projectData.cliente
                );
                setProjectData((prev) => ({
                  ...prev,
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          {sede.presupuesto.materiales.toLocaleString("es-CO", {
                            style: "currency",
                            currency: "COP",
                            minimumFractionDigits: 0,
                          })}
                      ...
                      <div className="flex justify-between">
                        <span>Servicios:</span>
                        <span className="font-semibold">
                          {sede.presupuesto.servicios.toLocaleString("es-CO", {
                            style: "currency",
                            currency: "COP",
                            minimumFractionDigits: 0,
                          })}
                      ...
                      <div className="flex justify-between col-span-2 border-t pt-1 font-bold text-gray-900">
                        ...
                        <span>
                          {sede.presupuesto.total.toLocaleString("es-CO", {
                            style: "currency",
                            currency: "COP",
                            minimumFractionDigits: 0,
                          })}
            ...
            <div>
              ...
              <div className="space-y-2">
                {materialesDisponibles.map((mat, idx) => {
                  const asignadoIdx = sedeForm.materialesAsignados.findIndex(
                    (m) => m.item === mat.item
                  );
                  ...
                  const cantidadEstaSede =
                    editingSede !== null
                      ? Number(
                          projectData.sedes[
                            editingSede
                          ].materialesAsignados.find((m) => m.item === mat.item)
                            ?.cantidad || 0
                        )
                      : 0;
                  const maxDisponible =
            ...
            <div>
              ...
              <div className="space-y-2">
                {serviciosDisponibles.map((serv, idx) => {
                  const asignadoIdx = sedeForm.serviciosAsignados.findIndex(
                    (s) => s.servicio === serv.servicio
                  );
                  const cantidadAsignada =
                    asignadoIdx !== -1
  ...
  return (
    <div
      className="fixed inset-0 bg-black bg-opacity-25 flex justify-center items-start z-50 p-4 pt-12"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  required
                />
                <span className="text-red-500 text-xs">{errors.fechaFin}</span>
              </div>
              <div>
              ...
              <div className="mt-2 max-h-32 overflow-y-auto border rounded bg-white shadow">
                {loadingUsuarios ? (
                  ...
                  usuarios
                    ...
                    .filter(
                      (u) =>
                        !projectData.empleadosAsociados.includes(
                          `${u.nombre} ${u.apellido}`
                        )
                    )
                    .map((u) => (
          ...
          <FormSection title="Costos Adicionales">
            <div>
              ...
              <input
                ...
                min="0"
              />
              <span className="text-red-500 text-xs">{errors.manoDeObra}</span>
            </div>
          </FormSection>
